...
vennTextPosition <- function (vennout, adj = c(0.5, 1)){
  ...
  # position of the main compartment
    ...
    textA <- r1 * vecRotate(c(1,0), thetaA-pi) * adj[1]
    textB <- c2p + (r2 * vecRotate(c(1, 0), thetaB_AC))  * adj[1]
    textC <- c3p + (c3p / sqrt(c3p[1]^2 + c3p[2]^2)) * r3 * adj[1]
  ...
  # position of the partially overlapped area
    ...
    if (max( c(r1,r2) ) < (AB$d + min( c(r1, r2) ) ) & AB$d < (r1 + r2)){
      xp <- vecRotate( c(AB$x, 0), AB$theta1) + c1p # mid point
      outtemp$textAB <- xp + vecRotate(c(0, max(c(r1, r2))), AB$theta0) * adj[2]
    }
  
    # if A and C partially overlap
    AC <- getTriangle(c3p, r3, c1p, r1)
    if (max( c(r1, r3) ) < (AC$d + min( c(r1, r3) ) ) & AC$d < (r1 + r3)){
      xp <- vecRotate( c(AC$x, 0), AC$theta1) + c3p # mid point
      outtemp$textAC <- xp + vecRotate(c(0, max(r1, r3)), AC$theta0) * adj[2]
    }
    
    # if B and C partially overlap
    BC <- getTriangle(c2p, r2, c3p, r3)
    if (max( c(r2, r3) ) < (BC$d + min( c(r2, r3) ) ) & BC$d < (r2 + r3)){
      xp <- vecRotate( c(BC$x, 0), BC$theta1) + c2p # mid point
      outtemp$textBC <- xp + vecRotate(c(0, max(r2, r3)), BC$theta0) * adj[2]
    }
